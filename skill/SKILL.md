...
- **Never bind-mount the Unix socket into the container.** A macOS Unix domain socket does not work across the container-VM boundary (OrbStack, Docker Desktop, any of them): the socket file shows up inside the container but connecting to it fails with `ConnectionRefusedError: [Errno 111]`. The bridge exists because of this; the old `-v ~/.config/herdr:/run/herdr` mount and `HERDR_SOCKET_PATH` env var have been removed from `~/.hermes/config.yaml`. Don't reintroduce them.
